...
namespace DaggerfallWorkshop.Game.UserInterface
{
    public class UncannyTalkWindow : DaggerfallTalkWindow
    {
        ...
        protected override void Setup()
        {
            ...
            mainPanel.Size = new Vector2(310, 102); // reference size is always vanilla df resolution
            mainPanel.HorizontalAlignment = HorizontalAlignment.Center;
            mainPanel.VerticalAlignment = VerticalAlignment.Bottom;

            portraitMode = UncannyUILoader.Instance.GetModSettings().GetInt("Dialogue", "PortraitMode");

            if (portraitMode > 0)
            {
                int portraitSize = portraitMode % 2 == 1 ? 64 : 32; // Option 1 and 3 are big size
                int portraitPos = portraitMode == 3 ? (310 - 155 - (portraitSize / 2)) : 310 - portraitSize; //Option 3 is centered
            ...
            if (portraitMode == 1)
                panelNameNPC.Position = new Vector2(310 - 128 - 64 - 4, -1);
            else if (portraitMode == 2)
                panelNameNPC.Position = new Vector2(310 - 128 - 32 - 4, -1);
            else
                panelNameNPC.Position = new Vector2(310 - 128, -1);

            panelNameNPC.Size = new Vector2(128, 10);

            labelNameNPC = new TextLabel();
            labelNameNPC.Position = new Vector2(0, 0);
            labelNameNPC.Size = new Vector2(128, 10);
